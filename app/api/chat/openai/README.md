...
- **GPT-5 Models**: Support for `gpt-5.5`, `gpt-5`, `gpt-5.4`, `gpt-5.4-mini`, and `gpt-5.4-nano`
...
- **Reasoning Effort**: Configurable reasoning effort levels (`none`, `low`, `medium`, `high`, `xhigh`)
...
- `model` (string, optional): OpenAI model name (defaults to `gpt-5.5`)
...
- **Web Search**: Uses shared Firecrawl or Jina tools when a provider key is configured
- **Citations**: Provider sources are included as Markdown links rather than Anthropic citation blocks
