...
// xAI is OpenAI-compatible via the Responses API
const xai = new OpenAI({
  apiKey: process.env.XAI_API_KEY || "",
  baseURL: "https://api.x.ai/v1",
  // Reasoning models can take a long time; xAI docs recommend up to 1 hour
  timeout: 3600 * 1000,
});

type XAIReasoningEffort = "low" | "medium" | "high";

interface XAIAPIRequest {
  message: string;
...
function normalizeXAIReasoningEffort(
  ...
  // grok-4.5 defaults to high; none/xhigh are not supported
  return "high";
}
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const reasoningEffort = normalizeXAIReasoningEffort(
        formData.get("reasoning_effort") as string
      );
      ...
      for (let i = 0; i < attachmentCount; i++) {
        ...
        const size = parseInt(
          (formData.get(`attachment-${i}-size`) as string) || "0"
        );
    ...
    const reasoningEffort = normalizeXAIReasoningEffort(rawReasoningEffort);
    ...
    const response = await getXAIResponse({
      history: messages,
      newUserMessage: message,
      context: context || null,
      attachments,
...
// Convert Anthropic tools to xAI/OpenAI function format
function convertToolsToXAI(webSearchEnabled: boolean): Tool[] {
...
async function buildXAIRequestConfig({
  history,
  newUserMessage,
  context,
  attachments = [],
...
When linking to app records in markdown, use the exact absolute URL returned by the tool. Do not rewrite it as a relative path.`;
  ...
  const initialInput: ResponseInputItem[] = [...openaiHistory, newUserContent];

  return {
    systemPrompt,
    initialInput,
    tools: convertToolsToXAI(webSearchEnabled),
  };
}
...
async function getXAIResponse({
  ...
  try {
    const effort = normalizeXAIReasoningEffort(reasoningEffort);
    const { systemPrompt, initialInput, tools } =
      await buildXAIRequestConfig({
        history,
        newUserMessage,
        context,
        attachments,
    ...
    while (maxIterations > 0) {
      signal?.throwIfAborted();
      // xAI rejects instructions + previous_response_id together; reuse the
      // prior turn's system prompt when continuing a response chain.
    ...
    if (finalResponse) {
      const content =
        finalResponse.output_text?.trim() || "No response generated";

      // Get the first successful result for legacy response format
      const firstSuccessfulResult = allToolResults.find(
        (result) => result.success
      );

      return {
        message: content,
        functionResult: firstSuccessfulResult
          ? { success: true, data: firstSuccessfulResult.data }
          : { success: false, error: "All tools failed" },
  ...
  const body = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        while (maxIterations > 0) {
          ...
          for await (const event of stream as AsyncIterable<StreamEvent>) {
            ...
            if (event.type === "response.output_text.delta" && event.delta) {
              ...
              isResponseFunctionToolCall(event.item)
            ) {
        ...
        const message =
          (finalMessage || finalResponse?.output_text || "").trim() ||
        ...
        send("done", {
          ...
          citations: [],
          actions: [],
        } satisfies XAIAPIResponse);
      } catch (error) {
        if (!signal?.aborted) {
          console.error("xAI API streaming error:", error);
          send("error", {
            message:
              error instanceof Error
                ? error.message
                : "Failed to get response from xAI API",
          });
        }
      } finally {
        closed = true;
